...
const toggleLike = async (req, res, targetType) => {
  ...
  if (existingLike) {
    ...
    await Like.create({
      [targetType]: targetId,
      likedBy: { userId, userType },
    });
    const likeCount = await calculateLikeCount(targetId, targetType);

    const senderName = req.traveler
      ? req.traveler.userName
      : req.agency?.userName;
    ...
    return res
      .status(201)
      .json(
        new ApiResponse(201, { likeCount }, `${targetType} liked successfully.`)
      );
  }
};
...
const toggleCommentLike = asyncHandler(async (req, res) =>
  toggleLike(req, res, "comment")
);


const toggleTweetLike = asyncHandler(async (req, res) =>
  toggleLike(req, res, "tweet")
);


const togglePackageLike = asyncHandler(async (req, res) =>
  toggleLike(req, res, "package")
);
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        likedItems,
        `Liked ${targetType}s fetched successfully.`
      )
    );
};
...
const getLikedPost = asyncHandler(async (req, res) =>
  getLikedItems(req, res, "post", "posts")
);


const getLikedPackage = asyncHandler(async (req, res) =>
  getLikedItems(req, res, "package", "packages")
);
